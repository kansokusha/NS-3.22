...
#include <ns3/tap-bridge-epc-helper.h>
#include <ns3/log.h>
...
NS_LOG_COMPONENT_DEFINE ("TapBridgeEpcHelper");

NS_OBJECT_ENSURE_REGISTERED (TapBridgeEpcHelper);

TapBridgeEpcHelper::TapBridgeEpcHelper () 
...
TypeId
TapBridgeEpcHelper::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TapBridgeEpcHelper")
    .SetParent<EpcHelper> ()
    .AddConstructor<TapBridgeEpcHelper> ()
    .AddAttribute ("sgwDeviceName", 
                   "The name of the device used for the S1-U interface of the SGW",
                   StringValue ("veth0"),
                   MakeStringAccessor (&TapBridgeEpcHelper::m_sgwDeviceName),
                   MakeStringChecker ())
    .AddAttribute ("enbDeviceName", 
                   "The name of the device used for the S1-U interface of the eNB",
                   StringValue ("veth1"),
                   MakeStringAccessor (&TapBridgeEpcHelper::m_enbDeviceName),
                   MakeStringChecker ())
    .AddAttribute ("SgwMacAddress", 
                   "MAC address used for the SGW ",
                   StringValue ("00:00:00:59:00:aa"),
                   MakeStringAccessor (&TapBridgeEpcHelper::m_sgwMacAddress),
                   MakeStringChecker ())
    .AddAttribute ("EnbMacAddressBase", 
                   "First 5 bytes of the Enb MAC address base",
                   StringValue ("00:00:00:eb:00"),
                   MakeStringAccessor (&TapBridgeEpcHelper::m_enbMacAddressBase),
                   MakeStringChecker ())
    ;
  return tid;
}
...
void
TapBridgeEpcHelper::DoInitialize ()
{
  NS_LOG_LOGIC (this);   
  ...
  // Create TapBridge for SGW
  NodeContainer ghostNode;
  ghostNode.Create (1);

  CsmaHelper csma;
  NetDeviceContainer csmaNetDevice = csma.Install (NodeContainer (m_sgwPgw, ghostNode.Get (0)));
  Ptr<NetDevice> sgwDevice = csmaNetDevice.Get (0);
  Ptr<NetDevice> ghostDevice = csmaNetDevice.Get (1);

  TapBridgeHelper tap;
  tap.SetAttribute ("Mode", StringValue ("UseBridge"));
  NS_LOG_LOGIC ("SGW device: " << m_sgwDeviceName);
  tap.SetAttribute ("DeviceName", StringValue (m_sgwDeviceName));
  tap.Install (ghostNode.Get (0), ghostDevice);
  ...
  m_sgwIpIfaces = m_epcIpv4AddressHelper.Assign (sgwDevice);
...
void
TapBridgeEpcHelper::AddEnb (Ptr<Node> enb, Ptr<NetDevice> lteEnbNetDevice, uint16_t cellId)
{
  ...
  // Create TapBridge for the eNB to connect with the SGW and other eNBs
  NodeContainer ghostNode;
  ghostNode.Create (1);

  CsmaHelper csma;
  NetDeviceContainer csmaNetDevice = csma.Install (NodeContainer (enb, ghostNode.Get (0)));
  Ptr<NetDevice> enbDev = csmaNetDevice.Get (0);
  Ptr<NetDevice> ghostDevice = csmaNetDevice.Get (1);
  ...
  tap.SetAttribute ("DeviceName", StringValue (enbDevNameWithCount.str()));  
  tap.Install (ghostNode.Get (0), ghostDevice);
  ...
  //tap.EnablePcap ("enbDev", enbDev);

  NS_LOG_LOGIC ("number of Ipv4 ifaces of the eNB after installing tap dev: " << enb->GetObject<Ipv4> ()->GetNInterfaces ());  
  Ipv4InterfaceContainer enbIpIfaces = m_epcIpv4AddressHelper.Assign (enbDev);
...
void
TapBridgeEpcHelper::AddX2Interface (Ptr<Node> enb1, Ptr<Node> enb2)
{
...
void 
TapBridgeEpcHelper::AddUe (Ptr<NetDevice> ueDevice, uint64_t imsi)
{
...
uint8_t
TapBridgeEpcHelper::ActivateEpsBearer (Ptr<NetDevice> ueDevice, uint64_t imsi, Ptr<EpcTft> tft, EpsBearer bearer)
{
...
Ptr<Node>
TapBridgeEpcHelper::GetPgwNode ()
{
  return m_sgwPgw;
}

Ipv4InterfaceContainer 
TapBridgeEpcHelper::AssignUeIpv4Address (NetDeviceContainer ueDevices)
{
  return m_ueAddressHelper.Assign (ueDevices);
}

Ipv4Address
TapBridgeEpcHelper::GetUeDefaultGatewayAddress ()
{
