...
main (int argc, char *argv[])
{
  ...
  // Setup IP base for PGW
  Ipv4Address pgwIp ("1.0.0.0");
  Ipv4Mask pgwMask ("255.0.0.0");
  Ipv4AddressHelper ipv4h;
  ipv4h.SetBase (pgwIp, pgwMask);

  // Create a TapFdNetDevice
  TapFdNetDeviceHelper tap;
  tap.SetDeviceName ("pgwTap");
  tap.SetTapMacAddress (Mac48Address::Allocate ());
  NetDeviceContainer pgwDevices = tap.Install (pgw);

  // Add an address to the NS-3 IPv4 interface
  Ipv4InterfaceContainer pgwIpIfaces = ipv4h.Assign (pgwDevices);
  NS_LOG_LOGIC ("IPv4 interface address of PGW: " << pgwIpIfaces.GetAddress (0, 0));
  ...
  Simulator::Schedule (Seconds (0.0), static_cast<void (LteHelper::*)(NetDeviceContainer)>(&LteHelper::Attach), lteHelper, ueLteDevs);
